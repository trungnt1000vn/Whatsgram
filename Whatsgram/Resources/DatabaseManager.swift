...
extension DatabaseManager {
    ...
    public func getAllMessagesForConversation(with id: String, completion: @escaping (Result<[Message],Error>) -> Void){
        database.child("\(id)/messages").observe(.value, with: { [self]snapshot in
            guard let value = snapshot.value as? [[String:Any]] else {
                completion(.failure(DatabaseError.failedToFetch))
                return
            }
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                let content = dictionary["content"] as? String ?? "Unknown"
                ...
                if type == "photo"{
                    ...
                    getAudioDuration(from: audioUrl){
                        ...
                        let audio = Audio(url: audioUrl, duration: audioDuration, size: CGSize(width: 100, height: 30))
                        kind = .audio(audio)
    ...
    public func getAudioDuration(from url: URL, completion: @escaping (TimeInterval?) -> Void) {
